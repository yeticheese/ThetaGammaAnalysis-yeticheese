...
def emd_analysis(signal, fs):
    """
    Remove spurious cycles using EMD Cycle analysis on a given signal
...
def pipeline(thetaSignal, phasic=True, **kwargs):

    # Load the parameters from the config file
    f_theta = (kwargs.pop("f_theta_lower", 4), kwargs.pop("f_theta_upper", 12))
    threshold_kwargs = kwargs.pop("threshold_kwargs", None)
    skip_threshold = kwargs.pop("n_skip", 2500)
